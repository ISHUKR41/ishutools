...
const PRESETS   = {
  quick:   { tip:'Fastest merge. Bookmarks preserved.',            bm:true,  toc:false, sep:false, comp:false, dd:false },
  report:  { tip:'TOC + separator pages for professional docs.',   bm:true,  toc:true,  sep:true,  comp:false, dd:false },
  compact: { tip:'Smallest output: compress + deduplicate.',       bm:false, toc:false, sep:false, comp:true,  dd:true  },
  archive: { tip:'All features on. Maximum quality archive.',      bm:true,  toc:true,  sep:true,  comp:true,  dd:false },
};
...
/* D is populated inside DOMContentLoaded — DO NOT use at module scope */
let D = null;
...
function toast(msg, type = 'info', dur = 3400) {
  if (!D) return;
  const ic = { success:'fa-circle-check', error:'fa-circle-xmark', warn:'fa-triangle-exclamation', info:'fa-circle-info' }[type] || 'fa-circle-info';
...
function pushUndo(entry, idx) {
  ...
  D.undoName.textContent = entry.name;
  D.undoBar.classList.add('show');
  clearTimeout(_undoTimer);
  _undoTimer = setTimeout(() => D.undoBar.classList.remove('show'), 5000);
}
...
function addFiles(fileList) {
  ...
  for (const f of Array.from(fileList)) {
    if (FILES.length >= MAX_FILES) { toast(`Max ${MAX_FILES} files allowed`, 'warn'); break; }
    if (f.size > MAX_BYTES) { toast(`"${f.name}" exceeds 1 GB`, 'error'); continue; }
    const ex = fileExt(f.name);
    if (!IMG_EXT.has(ex) && !PDF_EXT.has(ex)) { toast(`Unsupported: ${f.name}`, 'warn'); continue; }
    ...
    if (entry.imgConverted) genImgThumb(entry); else readPdfMeta(entry);
  }
...
async function readPdfMeta(entry) {
  ...
  try {
    ...
    entry.thumb = cv.toDataURL('image/jpeg', .74);
    refreshCard(entry); updateStats();
  } catch (err) {
    if (err?.name === 'PasswordException' || String(err).toLowerCase().includes('password')) entry.enc = true;
    refreshCard(entry);
  }
}
...
/* ════════ OUTPUT FILENAME ════════ */
function smartName() {
  const m = (D?.optFilename?.value || '').trim();
  if (m) return m.replace(/\.pdf$/i, '').trim() + '.pdf';
...
function buildCard(entry, idx) {
  ...
  if (entry.thumb) {
    ...
    const lbl = entry.imgConverted ? fileExt(entry.name).slice(1).toUpperCase() || 'IMG' : 'PDF';
  ...
  if (entry.pages !== null) pills += `<span class="fp"><i class="fas fa-book-open"></i>${entry.pages}pg</span>`;
  if (entry.enc)            pills += `<span class="fp enc"><i class="fas fa-lock"></i>Encrypted</span>`;
  if (entry.imgConverted)   pills += `<span class="fp" style="color:var(--wrn);border-color:rgba(245,158,11,.2)"><i class="fas fa-image"></i>→PDF</span>`;
  if (entry.pages !== null && !entry.enc) pills += `<span class="fp ok"><i class="fas fa-circle-check"></i>Ready</span>`;
  ...
  const rangeHtml = entry.imgConverted
    ...
    : `<div class="fc-field"><label><i class="fas fa-list-ol"></i>Page Range</label>
       <input type="text" class="fc-range-inp" placeholder="all / 1-3,5 / odd / even / first 2 / last 3" value="${entry.range || ''}"/>
  ...
  const pwdHtml = entry.enc
    ? `<div class="fc-field"><label><i class="fas fa-lock"></i>Password</label>
       <input type="password" class="fc-pwd-inp" placeholder="Enter PDF password" value="${entry.pwd || ''}" autocomplete="new-password"/></div>`
    : '';
    ...
    <div class="fc-info">
      ...
      <div class="fc-expand">
        <div class="fc-row">
          ...
          <input type="text" class="fc-dname-inp" placeholder="Name in TOC / separator" value="${entry.displayName || ''}"/></div>
        </div>
      </div>
    </div>
    <div class="fc-acts">
      ...
      <div class="fc-btns">
        ...
        <button class="fc-btn del" title="Remove"><i class="fas fa-trash"></i></button>
      </div>
    </div>
    <div class="swipe-reveal"><i class="fas fa-trash-alt"></i>Remove</div>`;
  ...
  div.querySelector('.fc-btn.del').addEventListener('click', e => { e.stopPropagation(); removeFile(entry.id); });
  ...
  div.addEventListener('keydown', e => {
    ...
    if (e.altKey && e.key === 'ArrowUp' && i > 0) {
      e.preventDefault(); [FILES[i], FILES[i-1]] = [FILES[i-1], FILES[i]];
...
function refreshCard(entry) {
  const old = D?.fList?.querySelector(`[data-id="${entry.id}"]`); if (!old) return;
  ...
  _sortable = Sortable.create(D.fList, {
    ...
    onEnd: ev => {
      ...
      D.fList.querySelectorAll('.fc[data-id]').forEach(c => {
        const e = FILES.find(x => x.id === c.dataset.id); if (e) reordered.push(e);
      });
      ...
      document.querySelectorAll('.sb').forEach(b => b.classList.toggle('active', b.dataset.sort === 'order'));
    },
  });
}
...
function removeFile(id) {
  ...
  if (card) {
    card.style.cssText = 'opacity:0;transform:translateX(12px) scale(.95);pointer-events:none;transition:.2s ease';
    setTimeout(() => card.remove(), 220);
  }
  updateStats();
  D.fileBadge.textContent = `${FILES.length} file${FILES.length !== 1 ? 's' : ''}`;
  if (FILES.length === 0) showSection('upload'); else updateMergeBtn();
}
...
function updateStats() {
  ...
  D.stEst.textContent   = FILES.length > 0 ? (est < 60 ? `~${Math.max(1, Math.round(est))}s` : `~${Math.round(est / 60)}m`) : '—';
  const big = FILES.filter(f => f.size > 100 * 1024 * 1024);
  D.largeBanner.hidden  = big.length === 0;
  if (big.length > 0) D.largeBanner.innerHTML = `<i class="fas fa-triangle-exclamation"></i> ${big.length} large file(s) detected — merge may take a moment`;
  updateMergeBtn();
}
...
function setProg(pct, title, sub) {
  pct = clamp(pct, 0, 100);
  D.ring.style.strokeDashoffset = CIRC - CIRC * pct / 100;
...
async function startMerge() {
  ...
  openSSE(_jobId);
  stepProg(0);
  setProg(6, 'Uploading…', `Sending ${FILES.length} file${FILES.length !== 1 ? 's' : ''} to server`);
  ...
  try {
    ...
    toast(err.message || 'Merge failed — check your files and try again', 'error', 7000);
    showSection('files');
...
function closeSSE() { if (_sse) { _sse.close(); _sse = null; } clearInterval(_simTimer); }
function simProgress() {
  let pct = 10; clearInterval(_simTimer);
  _simTimer = setInterval(() => {
    const cur = parseFloat(D.pbar.style.width || '0');
    if (cur < pct && pct < 82) { setProg(pct); stepProg(pct < 22 ? 0 : pct < 56 ? 1 : 2); }
    pct = Math.min(pct + (Math.random() * 3.5), 82);
    if (pct >= 82) clearInterval(_simTimer);
  }, 400);
}
...
function showResult(totalPages, srcCount, methodUsed, outputSize, skippedDups, blobSize) {
  ...
  const saved   = chg < -0.5 ? `−${Math.abs(chg).toFixed(1)}%` : chg > 0.5 ? `+${chg.toFixed(1)}%` : '0%';

  counterAnim(D.rFiles, srcCount);
  counterAnim(D.rPages, totalPages);
  D.rSize.textContent   = fmtSize(sz);
  D.rTime.textContent   = elapsed;
  D.rEngine.textContent = (methodUsed || 'pypdf').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  D.rSaved.textContent  = saved;
  D.rSaved.style.color  = chg < -1 ? 'var(--ok)' : chg > 3 ? 'var(--err)' : 'var(--tx)';
  D.resSub.textContent  = skippedDups > 0
    ? `${srcCount} files merged · ${totalPages} pages · ${skippedDups} duplicate page${skippedDups !== 1 ? 's' : ''} removed`
  ...
  D.dlBtn.onclick = () => {
    ...
    toast('Downloading…', 'success', 2400);
  };
  D.copyNameBtn.onclick = () => {
    navigator.clipboard?.writeText(_dlName).then(() => {
      ...
      setTimeout(() => { D.copyNameBtn.classList.remove('copied'); D.copyNameBtn.innerHTML = orig; }, 2300);
  ...
  _recentMerges.unshift({ id: genId(), name: _dlName, files: srcCount, pages: totalPages, size: fmtSize(sz), time: new Date().toLocaleTimeString() });
...
function counterAnim(el, target) {
  ...
  const t = setInterval(() => { v = Math.min(v + step, target); el.textContent = v; if (v >= target) clearInterval(t); }, 28);
}
    ...
    <div class="rec-wrap">
      ...
      <div class="rec-list">${_recentMerges.map(r => `
        <div class="rec-item">
          ...
          <div><div class="rec-name">${r.name}</div><div class="rec-meta">${r.files} files · ${r.pages} pages · ${r.size}</div></div>
...
function launchConfetti(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const sz = 6 + Math.random() * 10, dr = 2.2 + Math.random() * 2.4;
...
async function openPreview(entry) {
  ...
  if (entry.imgConverted) {
    const url = URL.createObjectURL(entry.file);
    D.pvBody.innerHTML = `<div class="pv-img-wrap"><img src="${url}" alt="${entry.name}"/></div>`;
    return;
  }
  if (typeof pdfjsLib === 'undefined') {
    D.pvBody.innerHTML = `<div class="pv-err"><i class="fas fa-triangle-exclamation"></i>PDF.js loading… try again in a moment</div>`;
    return;
  }
  try {
    const buf  = await entry.file.arrayBuffer();
    ...
    for (let i = 1; i <= maxPg; i++) {
      ...
      const vp = pg.getViewport({ scale: .64 });
    ...
    if (isPass) {
      ...
      D.pvBody.innerHTML = `<div class="pv-err"><i class="fas fa-lock"></i>${entry.pwd ? 'Wrong password' : 'Expand the file card and enter the PDF password'}</div>`;
    } else {
      D.pvBody.innerHTML = `<div class="pv-err"><i class="fas fa-triangle-exclamation"></i>${err.message || 'Cannot preview this file'}</div>`;
    }
  }
}
function closePreview() {
  if (!D) return;
  D.pvModal.hidden = true; document.body.style.overflow = '';
...
function setupDrop() {
  D.dz.addEventListener('click', e => {
    if (e.target.closest('.dz-pick') || e.target === D.dz || e.target.closest('.dz-icon') ||
        e.target.tagName === 'H2' || e.target.tagName === 'P') D.fi.click();
  });
  document.querySelector('.dz-pick')?.addEventListener('click', e => { e.stopPropagation(); D.fi.click(); });
  D.dz.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); D.fi.click(); } });
  ...
  D.dz.addEventListener('dragleave', e => { if (!D.dz.contains(e.relatedTarget)) D.dz.classList.remove('over'); });
  D.dz.addEventListener('drop', e => { e.preventDefault(); D.dz.classList.remove('over'); if (e.dataTransfer.files.length) addFiles(e.dataTransfer.files); });
  document.addEventListener('dragenter', e => { if (e.dataTransfer?.types?.includes('Files') && D.globalDrag) D.globalDrag.classList.add('on'); });
  document.addEventListener('dragleave', e => { if (D.globalDrag && (!e.relatedTarget || e.relatedTarget.nodeName === 'HTML')) D.globalDrag.classList.remove('on'); });
...
function addSwipe(card, id) {
  ...
  let tx0 = 0, dx = 0;
  card.addEventListener('touchstart', e => { tx0 = e.touches[0].clientX; dx = 0; }, { passive: true });
  card.addEventListener('touchmove', e => {
    dx = e.touches[0].clientX - tx0;
    if (dx < 0) {
      card.style.transform = `translateX(${Math.max(dx, -110)}px)`;
      if (hint) hint.style.opacity = String(Math.min(1, -dx / 55));
    }
  }, { passive: true });
  card.addEventListener('touchend', () => {
    if (dx < -92) removeFile(id);
...
function loadPdfJs() {
  ...
  s.onload = () => {
    ...
    FILES.filter(e => !e.imgConverted && e.pages === null).forEach(e => readPdfMeta(e));
  };
...
function initCanvas() {
  ...
  const N = Math.min(48, Math.floor(innerWidth / 28));
  const pts = Array.from({ length: N }, () => ({
    x: Math.random() * 1920, y: Math.random() * 1080,
    vx: (Math.random() - .5) * .2, vy: (Math.random() - .5) * .2,
    r: 1 + Math.random() * 1.5, a: .06 + Math.random() * .14,
  }));
  const CONN = 120;
  const frame = () => {
    ...
    for (let i = 0; i < pts.length; i++) for (let j = i + 1; j < pts.length; j++) {
      ...
      if (d < CONN) {
        ...
        cx.strokeStyle = `rgba(99,102,241,${.042 * (1 - d / CONN)})`; cx.lineWidth = .5; cx.stroke();
      }
    }
...
function initAnimations() {
  ...
  gsap.from('.hero-badge', { y: 18, duration: .65, delay: .06, ease: 'power3.out' });
  gsap.from('.hero h1',    { y: 26, duration: .78, delay: .18, ease: 'power3.out' });
  gsap.from('.hero-desc',  { y: 18, duration: .64, delay: .3,  ease: 'power3.out' });
  gsap.from('.hero-chips', { y: 12, duration: .6,  delay: .42, ease: 'power3.out' });
  gsap.from('#secUpload',  { y: 24, duration: .7,  delay: .5,  ease: 'power3.out' });
  if (typeof ScrollTrigger !== 'undefined') {
    ...
    ].forEach(({ sel, trg }) => gsap.from(sel, {
      y: 30, duration: .62, stagger: .09, ease: 'power3.out',
...
/* ══════════════════════════════════════
   INIT — all event bindings happen here
   ══════════════════════════════════════ */
document.addEventListener('DOMContentLoaded', () => {
  /* ── Populate D ── */
  D = {
    sUp: $('secUpload'), sFi: $('secFiles'), sPr: $('secProgress'), sRe: $('secResult'),
    dz: $('dropZone'), fi: $('fileInput'), addMoreBtn: $('addMoreBtn'), addMore: $('addMoreInput'), clearBtn: $('clearBtn'),
    ...
    optMethod: $('optMethod'), optFilename: $('optFilename'), optTitle: $('optTitle'), optAuthor: $('optAuthor'),
    optToc: $('optToc'), optSep: $('optSep'), optBookmarks: $('optBookmarks'),
    optCompress: $('optCompress'), optDedup: $('optDedup'), optNorm: $('optNorm'),
    optTargetSize: $('optTargetSize'), normSzField: $('normSzField'),
    mergeBtn: $('mergeBtn'), mCount: $('mCount'),
    ring: $('ringFg'), pbar: $('pbar'), progTitle: $('progTitle'), progSub: $('progSub'),
    ps1: $('ps1'), ps2: $('ps2'), ps3: $('ps3'), ps4: $('ps4'),
    resSub: $('resSub'), resFn: $('resFn'), resFnTx: $('resFnTx'),
    rFiles: $('rFiles'), rPages: $('rPages'), rSize: $('rSize'), rTime: $('rTime'), rEngine: $('rEngine'), rSaved: $('rSaved'),
    dlBtn: $('dlBtn'), copyNameBtn: $('copyNameBtn'), mergeAgainBtn: $('mergeAgainBtn'), recentMerges: $('recentMerges'),
  ...
  D.themeIcon.className = saved === 'dark' ? 'fas fa-moon' : 'fas fa-sun';
  ...
  D.soundToggle.addEventListener('click', () => { window.SOUNDS?.toggle?.(); updateSoundBtn(); });
  updateSoundBtn();

  /* ── Navbar scroll ── */
  window.addEventListener('scroll', () => D.navbar.classList.toggle('scrolled', scrollY > 38), { passive: true });

  /* ── Sort buttons ── */
  document.querySelectorAll('.sb').forEach(btn => btn.addEventListener('click', () => {
    _sortMode = btn.dataset.sort;
    document.querySelectorAll('.sb').forEach(b => b.classList.toggle('active', b.dataset.sort === _sortMode));
  ...
  document.querySelectorAll('.pre-btn').forEach(btn => {
    btn.addEventListener('mouseenter', () => { const p = PRESETS[btn.dataset.p]; if (p) D.preTip.textContent = p.tip; });
    ...
    btn.addEventListener('click', () => {
      ...
      D.optBookmarks.checked = p.bm; D.optToc.checked = p.toc; D.optSep.checked = p.sep;
      D.optCompress.checked  = p.comp; D.optDedup.checked = p.dd;
      window.SOUNDS?.playPresetSound?.();
      toast(`Preset: ${btn.dataset.p.charAt(0).toUpperCase() + btn.dataset.p.slice(1)}`, 'info', 1800);
    });
  });
  ...
  D.optNorm.addEventListener('change', () => { D.normSzField.hidden = !D.optNorm.checked; });
  ...
  D.clearBtn.addEventListener('click', () => {
    ...
    window.SOUNDS?.playFileRemoveSound?.();
    toast('All files cleared', 'info', 2000);
  });
  ...
  D.kbdBtn.addEventListener('click', () => { D.kbdModal.removeAttribute('hidden'); window.SOUNDS?.playExpandSound?.(); });
  ...
  document.addEventListener('keydown', e => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 's') { e.preventDefault(); if (_dlUrl) D.dlBtn?.click(); return; }
  ...
  document.querySelectorAll('.faq-q').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.faq.open').forEach(i => { i.classList.remove('open'); i.querySelector('.faq-q').setAttribute('aria-expanded','false'); });
