...
# ═══════════════════════════════════════════════════════════════════════════════
# ENTERPRISE ADVANCED FUNCTIONS - pdf_ocr.py
# ═══════════════════════════════════════════════════════════════════════════════

def ocr_extract_to_json(input_path: str, language: str = 'eng') -> dict:
    """OCR PDF and return structured JSON with text, bounding boxes, confidence per page."""
    ...
    for page_num, page in enumerate(doc):
        ...
        words = []
        for i, word in enumerate(data['text']):
            if word.strip() and int(data.get('conf', [0]*len(data['text']))[i]) > 30:
                words.append({
                    'text': word,
    ...
    doc.close()
    return {'page_count': len(doc) if not doc.is_closed else len(pages_data), 'pages': pages_data}

def ocr_pdf_to_txt(input_path: str, output_txt_path: str, language: str = 'eng') -> dict:
    """OCR PDF and save extracted text to a .txt file."""
    import fitz, pytesseract
    from PIL import Image
    doc = fitz.open(input_path)
    all_text = []
    ...
    with open(output_txt_path, 'w', encoding='utf-8') as f:
        f.write(full_text)
    return {'output_path': output_txt_path, 'page_count': len(all_text), 'total_chars': len(full_text), 'word_count': len(full_text.split())}

def ocr_detect_language(input_path: str, page_num: int = 0) -> dict:
    """Detect language of scanned PDF using OCR and langdetect."""
    import fitz, pytesseract
    from PIL import Image
    try:
    ...
    if has_langdetect and text.strip():
        try: detected = detect(text)
        except: pass
    return {'page': page_num+1, 'sample_text': text[:500], 'detected_language': detected, 'word_count': len(text.split())}

def ocr_pdf_pages(input_path: str, output_path: str, page_numbers: list, language: str = 'eng') -> dict:
    """OCR only specific pages of a PDF and return searchable PDF."""
    import fitz, pytesseract
    from PIL import Image
    import io
    doc = fitz.open(input_path)
    ...
    return {'output_path': output_path, 'ocr_pages': len(page_numbers), 'total_output_pages': total}

def ocr_enhance_and_process(input_path: str, output_path: str, language: str = 'eng', enhance: bool = True) -> dict:
    """OCR with image enhancement (contrast, sharpness, denoising) for better accuracy."""
    import fitz, pytesseract
    ...
    for page_num, page in enumerate(doc):
        ...
        pdf_bytes = pytesseract.image_to_pdf_or_hocr(img, lang=language, extension='pdf', config='--psm 3 --oem 3')
        sub_doc = fitz.open(stream=pdf_bytes, filetype="pdf")
        out_doc.insert_pdf(sub_doc)
        total_words += len(pytesseract.image_to_string(img, lang=language).split())
    out_doc.save(output_path, garbage=4, deflate=True)
    out_doc.close(); doc.close()
