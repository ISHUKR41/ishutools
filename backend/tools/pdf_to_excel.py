...
def extract_tables_camelot(input_path: str, output_xlsx_path: str,
                            ...
                            password: str = '') -> dict:
    """
    Extract tables using camelot-py (best for bordered/lattice tables).

    Args:
        flavor: 'lattice' (bordered) or 'stream' (whitespace-separated)
        pages: Page spec ('all', '1,3,5-8')
    """
    import camelot
    import openpyxl
    ...
    if not tables:
        raise ValueError('No tables detected with camelot. Try flavor="stream".')

    wb = openpyxl.Workbook()
    wb.remove(wb.active)

    for i, tbl in enumerate(tables):
        ws = wb.create_sheet(title=f'Table_{i+1}_P{tbl.page}')
        df = tbl.df
    ...
    return {
        ...
        'accuracy_scores': [round(t.accuracy, 1) for t in tables],
    }


def extract_tables_tabula(input_path: str, output_xlsx_path: str,
                           pages: str = 'all',
                           lattice: bool = False,
                           password: str = '') -> dict:
    """
    Extract tables using tabula-py (Java-based, excellent for complex tables).

    Args:
        pages: Page spec ('all', '1,3,5')
        lattice: True for bordered tables, False for stream-mode
    """
    import tabula
    import openpyxl

    dfs = tabula.read_pdf(input_path, pages=pages, multiple_tables=True,
                          lattice=lattice, stream=not lattice,
                          password=password or None)
    if not dfs:
        raise ValueError('No tables detected with tabula-py.')
        ...
        for r_idx, row in enumerate(df.itertuples(index=False), 2):
            for c_idx, val in enumerate(row, 1):
                ws.cell(row=r_idx, column=c_idx,
                        value=None if str(val) in ('nan', 'NaN', 'None') else val)
    ...
    wb.save(output_xlsx_path)
    return {'tables_found': len(dfs), 'output_path': output_xlsx_path}


def extract_tables_pdfplumber(input_path: str, output_xlsx_path: str,
                               pages: str = 'all',
                               password: str = '') -> dict:
    """
    Extract tables using pdfplumber (pure Python, great for text-based tables).

    pdfplumber is excellent for PDFs generated by Excel/Word where tables have
    no visible borders but are aligned by whitespace.
    """
    import pdfplumber
    import openpyxl
    ...
    with pdfplumber.open(input_path, password=password or None) as pdf:
        ...
        if pages != 'all':
            try:
                ...
                for part in pages.split(','):
                    ...
                    if '-' in part:
                        a, b = part.split('-')
                        nums.extend(range(int(a) - 1, int(b)))
                    else:
                        nums.append(int(part) - 1)
                page_range = [n for n in nums if 0 <= n < len(pdf.pages)]
            except Exception:
                pass
        ...
        for pg_idx in page_range:
            ...
            tables = pg.extract_tables()
            for t_idx, table in enumerate(tables):
                ws = wb.create_sheet(title=f'P{pg_idx+1}_T{t_idx+1}')
                for r_idx, row in enumerate(table, 1):
                    for c_idx, val in enumerate(row or [], 1):
                        ws.cell(row=r_idx, column=c_idx,
    ...
    wb.save(output_xlsx_path)
    return {'tables_found': table_count, 'output_path': output_xlsx_path}


def extract_all_text_to_excel(input_path: str, output_xlsx_path: str,
                               password: str = '') -> dict:
    """
    Extract ALL text from every page of a PDF into an Excel workbook.
    Each page becomes one row with: Page, Word Count, Text Content.

    Useful for text mining, NLP preprocessing, and document analysis.
    """
    import pdfplumber
    import openpyxl
    from openpyxl.styles import Font, PatternFill, Alignment
    ...
    # Header
    headers = ['Page', 'Word Count', 'Char Count', 'Text']
    ...
    with pdfplumber.open(input_path, password=password or None) as pdf:
        for pg_idx, pg in enumerate(pdf.pages):
            ...
            total_words += words
            r = pg_idx + 2
            ws.cell(row=r, column=1, value=pg_idx + 1)
            ws.cell(row=r, column=2, value=words)
            ws.cell(row=r, column=3, value=len(text))
            ws.cell(row=r, column=4, value=text[:5000])  # limit cell size
            ws.row_dimensions[r].height = min(max(15, words // 5), 120)

    wb.save(output_xlsx_path)
    return {
        'pages': len(wb['Extracted Text'].max_row) - 1,
        'total_words': total_words,
        'output_path': output_xlsx_path,
    }
