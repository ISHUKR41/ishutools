...
def compare_with_pdfplumber(path1: str, path2: str) -> dict:
    """
    Compare two PDFs using pdfplumber for detailed text-level comparison.
    Returns per-page diff summary with added/removed/changed sentences.
    """
    try:
        import pdfplumber
        ...
        sents1 = get_sentences(path1)
        sents2 = get_sentences(path2)
        added = list(sents2 - sents1)[:20]
        removed = list(sents1 - sents2)[:20]
        common = len(sents1 & sents2)
        total = len(sents1 | sents2)
        similarity = round(common / max(total, 1) * 100, 1)
        return {
            'similarity_pct': similarity,
            'sentences_added': len(added), 'examples_added': added[:5],
            'sentences_removed': len(removed), 'examples_removed': removed[:5],
            'common_sentences': common, 'total_unique': total,
        }
    except Exception as e:
        return {'error': str(e)}


def generate_html_diff_report(path1: str, path2: str, output_html_path: str) -> dict:
    """
    Generate a color-coded HTML diff report comparing two PDFs.
    Green = added, Red = removed, Gray = unchanged.
    """
    diff_data = compare_with_pdfplumber(path1, path2)
    html = f'''<!DOCTYPE html>
<html><head><title>PDF Comparison Report — IshuTools.fun</title><style>
body{{font-family:Arial;padding:30px;max-width:900px;margin:0 auto;}}
h1{{color:#6366f1;}} .added{{background:#d1fae5;padding:6px;margin:4px 0;border-left:4px solid #10b981;border-radius:4px;}}
.removed{{background:#fee2e2;padding:6px;margin:4px 0;border-left:4px solid #ef4444;border-radius:4px;}}
.stat{{display:inline-block;background:#f3f4f6;padding:8px 16px;border-radius:8px;margin:6px;font-weight:bold;}}
.score{{font-size:40px;color:#6366f1;font-weight:bold;}}
</style></head>
<body>
<h1>📄 PDF Comparison Report</h1>
<p>Generated by <strong>IshuTools.fun</strong> — Free PDF Tools by Ishu Kumar (ISHUKR41)</p>
<div class="score">{diff_data.get("similarity_pct", 0)}% Similar</div>
<div>
  <span class="stat">✅ Added: {diff_data.get("sentences_added", 0)}</span>
  <span class="stat">❌ Removed: {diff_data.get("sentences_removed", 0)}</span>
  <span class="stat">🔄 Common: {diff_data.get("common_sentences", 0)}</span>
</div>
<h2 style="color:#10b981">Added Content (in new version)</h2>
{"".join(f'<div class="added">{s}</div>' for s in diff_data.get("examples_added", [])[:10]) or '<p style="color:#888">No additions found</p>'}
<h2 style="color:#ef4444">Removed Content (from original)</h2>
{"".join(f'<div class="removed">{s}</div>' for s in diff_data.get("examples_removed", [])[:10]) or '<p style="color:#888">No removals found</p>'}
<hr><p style="color:#aaa;font-size:12px">IshuTools.fun — Free PDF Comparison Tool by Ishu Kumar | ishutools.fun</p>
</body></html>'''
