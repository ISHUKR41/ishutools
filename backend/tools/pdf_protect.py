...
def add_permissions_metadata(input_path: str, output_path: str,
                              allow_print: bool = True,
                              allow_copy: bool = False,
                              allow_modify: bool = False,
                              allow_annotations: bool = True) -> dict:
    """
    Set granular PDF permission flags without encrypting.
    Restricts operations like printing, copying, editing at the PDF level.
    """
    import pikepdf, os
    ...
    with pikepdf.open(input_path) as pdf:
        ...
        encryption = pikepdf.Encryption(
            user='', owner=os.urandom(16).hex(),
            allow=permissions, R=6, aes=True,
        )
        pdf.save(output_path, encryption=encryption)
...
def add_pdf_expiry(input_path: str, output_path: str,
    ...
    with pikepdf.open(input_path) as pdf:
        with pdf.open_metadata() as meta:
            meta['xmp:MetadataDate'] = expiry_str
            meta['dc:description'] = (
                message or f'This PDF is valid until {expiry.strftime("%Y-%m-%d")}. '
                f'Generated by IshuTools.fun by Ishu Kumar.'
            )
...
def verify_pdf_integrity(input_path: str) -> dict:
    """
    Verify PDF file integrity, check for corruption, validate structure.
    Returns detailed report of PDF health.
    """
    import pikepdf, fitz, hashlib, os
    results = {
    ...
    try:
        with pikepdf.open(input_path) as pdf:
            ...
            results['is_encrypted'] = pdf.is_encrypted
        ...
        results['issues'].append(f'pikepdf: {e}')
    except Exception as e:
        results['issues'].append(f'pikepdf error: {e}')
    ...
    try:
        ...
        results['issues'].append(f'fitz: {e}')
